...
{   
   ...
   {     
      ...
      {
	      $res = $this->eliminar_datos_core();
	      /* ¿hay que eliminar los datos de presupuestos y pedidos? */
	      if (in_array('presupuestos_y_pedidos', $GLOBALS['plugins']))
	      	$res = $res && $this->eliminar_datos_presupuestos_y_pedidos();	      
   ...
   private function eliminar_datos_core()
   {
   		$res = true;
   ...
   private function eliminar_datos_presupuestos_y_pedidos()
   {
   		require_model('db_cleaner_pres_y_ped.php');
   		$res = true;
